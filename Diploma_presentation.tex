...
\begin{enumerate}
 \item Измерить текущего состояния объекта $x_{t_i}$
  \item Вычислить оптимального (программного) управления $u^0(t)$, $t\in [t_i , t_{i} + T]$, --- задачи ${\cal P} (t_i , x_{t_i} ,T)$
  \item Управление $u^* (t): = u^0(t)$ при  $t\in [t_i , t_{i}+\delta]$ подать на вход объекта управления
  \item Процедуру повторить для следующего момента $t_{i+1}$
...
\includegraphics[width=1\linewidth]{MPCstat.png}
\label{fig:mpc}
...
\begin{alertblock}{}
При выполнении предположений, ряда условий устойчивости и  для достаточно малого периода квантования $\delta$  система, замкнутая обратной связью MPC, асимптотически устойчива в том смысле, что $ \|x^* (t)\| \rightarrow 0$ при $t \rightarrow \infty$
...
\begin{itemize}
  \item  В задаче (\ref{41}) заменим бесконечный горизонт управления конечным
...
$$
I_3(z,u) =\alert{\alpha e^{-\rho(\tau+T) } z(\tau+T)} + \int_\tau^{\tau+T} e^{-\rho t} [\kappa  \ln z(t) + \ln (b-u(t))]dt\rightarrow\max$$
