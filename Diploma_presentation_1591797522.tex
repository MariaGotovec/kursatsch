...
\begin{itemize}
\item множество $X_0 \subset \R^n$ возможных начальных состояний в    момент $t_0$

\item $x_t$ --- состояние объекта в момент времени $t \geq t_0$
...
\item $f ( 0, 0, t) = 0$, $t\ge t_0$, т.е. $x\equiv 0$ при $u\equiv 0$ --- положение равновесия уравнения (\ref{exg})
...
%
%  \begin{itemize} \itemsep = 4mm
%    \item Для всех $t\in \R$ множество $U (t)$ содержит начало координат и
%
...
\structure{\large Отличия от стабилизирующего МРС}

\begin{itemize}

\item В задаче стабилизации $L$ --- параметр настройки МРС-регулятора

\item Во многих приложениях $L$ задается исходя из экономических требований к процессу

\item Положение равновесия теперь определяется не из условия $ f (x,u)=0$, а из задачи нелинейного программирования
...
\item Решение задачи (\ref{23}) обозначается $(x_1, u_1)$ и в экономических задачах является магистралью
