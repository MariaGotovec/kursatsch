...
\end{lstlisting}
\end{itemize}
Для решения   нужно добавить после данного блока(не уверена что это так, но других идей нет)
  \begin{lstlisting}
    J=J+-exp(-rho*T)*psis*z(N)
      \end{lstlisting}

После формулировки задачи нелинейного программирования и ограничений, задача (\ref{41}) решается стандартной процедурой solver :
...
$$(z(t),p(t)) \in Г_1,$$
где
$$Г_1 = {(z,p) \in \R^2: z>0, \  p\geq \overline{h}(z)},$$
...
$$(z(t),p(t)) \in Г_2,$$
где
$$Г_2 = {(z,p) \in \R^2: z>0, \  0<p<\overline{h}(z)},$$
